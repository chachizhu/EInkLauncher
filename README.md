...
- 隐藏系统顶部状态栏：右上角首行只显示电量，下一行显示日期与固定 24 小时制时间；时间不显示秒。
- 不使用页面转场、列表动画、涟漪或平滑滚动。
- 只有用户在管理页手动检查更新或确认下载时才联网；不在启动时、定时或后台检查更新。
- 不运行后台服务、前台服务或周期任务。
- 时间、日期和电量只在桌面可见期间根据系统事件更新，不通过高频计时器轮询。
...
- 长按主屏空白区域约 1 秒，进入应用管理页。
- 也可以轻触顶部的时间、电量或日期区域直接进入设置。
- 带实体 Menu 键的设备也可以按 Menu 键进入管理页。
- 在管理页添加、移除或调整应用顺序，完成后返回主屏。
- 轻触“已选择”列表中的应用名称，可以设置仅用于主屏显示的简短别名。
- 在管理页选择“紧凑”“舒适”或“大字”显示预设，字号、行高和字重会一起调整并自动保存。
...
1. 轻触顶部的时间、电量或日期区域，或长按主屏空白区域进入设置，然后点击 **检查更新**。
